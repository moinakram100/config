sap.ui.define(
   ...
   */
  function (Controller,JSONModel,Fragment,Filter,FilterOperator,FilterType,MessageBox,ID) {
    "use strict";
    var parameterData;
    ...
    return Controller.extend("com.ingenx.config.controller.profileParameter", {
      onInit: function () {
        this.getOwnerComponent().getRouter().getRoute("RouteprofileParameter")
      ...
      handleRouteMatched: function (oEvent) {
        ...
        var oFilter = new Filter("serviceProfileName",FilterOperator.EQ,
          oEvent.getParameter("arguments").serviceProfileName
        );
        this.getView().byId("profileParameterTable").getBinding("items").filter(oFilter, FilterType.Application);
        ...
        try {
          ...
          console.log("Error reading serviceParametersItems data: ",error);
        }
      },
      ...
      onRefreshParameter: function () {
        ...
        Promise.all([this.addParameter(), this.parameterDataState()]).then(
          function () {
            var extraObjects = parameterModelData.filter(function (paramObject) {
              var profileName = that.getView().getModel("profileModel").getProperty("/ProfileName");
              return !proParameterModelData.some(function (proParamObject) {
                return (
                  proParamObject.serviceParameter ===paramObject.serviceParameter &&
            ...
            extraObjects.forEach(function (extraObject) {
              if (extraObject) {
                var oEntrySet = {
                  ...
                  serviceProfileName: that.getView().getModel("profileModel").getProperty("/ProfileName") || "",
                  serviceProfileDesc: that.getView().getModel("profileModel").getProperty("/ProfileDesc") || "",
                  ProfileId: that.getView().getModel("profileModel").getProperty("/ProfileId")||"",
                  ID: extraObject.ID || 0,
                  ...
                  serviceParameterlength: extraObject.serviceParameterlength || 0,
                  ParentId: "",
      ...
      addParameter: function () {
        ...
        return new Promise(function (resolve, reject) {
          ...
          that.getView().setModel(proParameterDataModel, "proParameterDataModel");
          ...
          oBindList.requestContexts(0, Infinity).then(function (aContexts) {
            ...
            that.getView().setModel(proParameterDataModel, "proParameterDataModel");
            proParameterModelData = that.getView().getModel("proParameterDataModel").getData();
            resolve();
          });
        });
      },
      ...
      parameterDataState: function () {
        ...
        return new Promise(function (resolve, reject) {
          ...
          oBindList.requestContexts(0, Infinity).then(function (aContexts) {
            ...
            that.getView().setModel(parameterDataModel, "parameterDataModel");
            parameterModelData = that.getView().getModel("parameterDataModel").getData();
            resolve();
          });
        });
      },
      ...
      RefreshData: function () {
        this.getView().byId("profileParameterTable").getBinding("items").refresh();
      },

      // Checking Allocation Level
      onSelectAllocRel: function (oEvent) {
        let selectedParameterObj = oEvent.getSource().getBindingContext().getObject();
